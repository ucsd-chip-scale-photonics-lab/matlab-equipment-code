classdef NKTControl < handle
    ...
    % select methods:
    % * none crucial, may add later
    % select RF driver methods:
    % * enableRf
    % * setRFWavelength
    % * setRFPower
    % * getWavelengthRange
    ...
    %   to come
    %
    % Original Authors: Jess Rigley and Villads Egede Johansen (University of Cambridge)
    ...
    % 2025 Author: Karl Johnson (University of California San Diego)
...
%             % obtains the status of the equipment at given address
%             statusRegister='66';
%             obj.sendTelegram(addr,obj.msgRead,statusRegister);
...
%         end
        ...
        % SELECT rf driver methods
        % all of these have an 'RFChannel' option to select which RF source
        % also note that all of these are natively in units of pm!
        % we convert to and from nm for interfacing

        function addr = getRFAddr(obj, RFChannel)
            if RFChannel == 1, addr = obj.addrRF1; else, addr = obj.addrRF2; end
        end

        % * enableRf
        function [] = enableRF(obj, doEnable, options)
            arguments
                obj
                doEnable
                options.RFChannel = 1
        ...
        % * getWavelengthRange
        function [lowerLambda, upperLambda] = getWavelengthRange(obj, options)
            arguments
                obj
                options.RFChannel = 1
            end
            thisAddr = getRFAddr(obj, options.RFChannel);
            lowerLambda = 1e-3*readRegister(obj, thisAddr, '34', 'uint32');
            upperLambda = 1e-3*readRegister(obj, thisAddr, '35', 'uint32');
        end

        % * setRFWavelength
        function replyLambda = setRFWavelength(obj, lambdaSet, options)
            arguments
                obj
                lambdaSet
                options.RFChannel = 1
            ...
            [lowerLambda, upperLambda] = getWavelengthRange(obj, ...
                RFChannel=options.RFChannel);
            if(lambdaSet < lowerLambda || lambdaSet > upperLambda)
                error('Desired wavelength %f outside limits [%f, %f]', lambdaSet, lowerLambda, upperLambda);
            end
            ...
            thisAddr = getRFAddr(obj, options.RFChannel);
            sendRegister = dec2hex(hex2dec('90') + options.WavelengthChannel);
            obj.writeRegister(thisAddr, sendRegister, 1e3*lambdaSet, 'uint32')
            % check it
            replyLambda = 1e-3*obj.readRegister(thisAddr, sendRegister, 'uint32');
            LAMBDA_TOLERANCE = 1e-3; % we accept up to 2 pm deviation
            if(abs(replyLambda-lambdaSet) > LAMBDA_TOLERANCE)
                warning('Wavelength response from SELECT %f not equal to specified wavelength %f', replyLambda, lambdaSet);
            end
        end
        ...
        function [] = writeRegister(obj, address, register, value, type)
            ...
            % make into a column of hex bytes with proper byte order
...
%                     end
    ...
    methods(Access=private)
        ...
        function [] = sendTelegram(obj,address,msgType,data)
            ...
            write(obj.s,hex2dec(out),'uint8'); %send to laser
        end
        ...
        function out = getTelegram(obj,size)
            ...
            received=read(obj.s,size, 'uint8');
            ...
            while out(length(out),:)~=obj.endTel 
                out(length(out)+1,:)=dec2hex(read(obj.s,1, 'uint8'),2);
            end
        end 
